...
\usepackage[ngerman]{babel}\addto\captionsngerman{
  \renewcommand{\lstlistingname}{Programmausschnitt}
  \renewcommand{\lstlistlistingname}{Programmausschnittverzeichnis}
}
...
und ihre Interferenz gezielt zu nutzen, um einen gewünschten Reflexions- und
Transmissionsgrad zu erreichen. Dünnschichtsysteme sind ein wichtiger
Bestandteil im Bereich der Optik und haben auch in anderen Fachgebieten einige
Anwendungen, z.B.:
\begin{itemize}
...
Im Folgenden wird Weg zur Berechnung des Reflexionsgrades beschrieben, um ein
möglichst realistisches Programm zur Erfassung eben dieses zu entwickeln. Das
größte Problem, das es hier zu lösen gilt, ist die Berücksichtigung aller
Teilwellen die bei jedem Grenzübergang, wie in Abbildung~\ref{fig:Reflexion} zu
sehen ist, innerhalb des Schichtsystems entstehen.

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.4]{./assets/Teilwellen.pdf}
...
Um das erwähnte Problem zu lösen wird die Transfermatrixmethode angewandt,
welche es ermöglicht auch unter Berücksichtigung der Teilwellen einen
Reflexionsgrad zu bestimmen. Bevor die Transfermatrixmethode angewandt werden
kann, müssen noch andere Eigenschaften ermittelt werden, auf welche in den
folgenden Unterkapiteln eingegangen wird.
\subsection{Brechungsindex}
Der komplexe Brechungsindex einer Schicht ist definiert als:
\begin{equation}
    \tilde{n} = n + i\kappa.
    \label{eq:Brechungsindex}
\end{equation}
Dabei beschreibt der Realteil $n$ die Phasengeschwindigkeit und der
Imaginärteil $\kappa$ die Absorption innerhalb des Mediums. Die
Wellenausbreitung in $z$-Richtung lässt sich durch Einsetzen von
Gleichung~\eqref{eq:Brechungsindex} im folgenden Ansatz darstellen:
\begin{equation}
    E(z) \propto e^{i\frac{\tilde{n}\omega}{c}z} = \underbrace{e^{i\frac{n\omega}{c}z}}_{\text{Phase}} \cdot \underbrace{e^{-\frac{\kappa\omega}{c}z}}_{\text{Dämpfung}}.
    \label{eq:Phasenverschiebung}
\end{equation}
Diese Aufteilung ist fundamental für die Aufstellung der Propagationsmatrix innerhalb der Transfermatrixmethode.
...
Messwertetabellen genutzt.~\cite{Kren:11}\cite{Malitson:65}.
...
\begin{equation}
    r_s = \frac{n_1 \cos\theta_1 - n_2 \cos\theta_2}{n_1 \cos\theta_1 + n_2
        \cos\theta_2}, \quad t_s = \frac{2n_1 \cos\theta_1}{n_1 \cos\theta_1 + n_2
        \cos\theta_2}
    \label{eq:polsenkrecht}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:Matrixgleichungssystem}
\end{equation}
geschrieben werden. Wird nach $\begin{pmatrix}R_i \\ L_i\end{pmatrix}$ aufgelöst erhält man
\begin{equation}
    \begin{pmatrix} R_i \\ L_i \end{pmatrix} =
    \frac{1}{t_{i,i+1}}
    \begin{pmatrix} 1 & -r_{i+1,i} \\ r_{i,i+1} & t_{i,i+1} t_{i+1,i} - r_{i,i+1} r_{i+1,i} \end{pmatrix}
    \begin{pmatrix} R_{i+1}^{\prime} \\ L_{i+1}^{\prime} \end{pmatrix}
    \label{eq:MatrixgleichungssystemRL}
\end{equation}
was das Muster der benötigten Transfermatrix liefert. In
Abbildung~\ref{fig:Gleichungssystem} befindet sich eine grafische Darstellung
der Herleitung.

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.3]{assets/Fresnel Gleichungssystem.pdf}
    \centering
    \caption{Abbildung der Herleitung des Gleichungssystems}\label{fig:Gleichungssystem}
\end{figure}

Es werden immer jeweils die zwei bereits erwähnten Arten von Matrizen gebildet,
dabei bestimmt die Grenzflächenmatrix D die Reflexion und Transmission an der
Grenzfläche, während die Propagationsmatrix P die Phasenverschiebung innerhalb
einer Schicht beschreibt. Die Grenzflächenmatrix besitzt die Form
\begin{equation}
    D = \frac{1}{t}\begin{pmatrix}
        1 & r \\
        r & 1
    \end{pmatrix},
    \label{eq:Grenzflaechenmatrix}
\end{equation}
...
Schicht sind. Daraufhin werden wie in der vorherigen Herleitung alle Matrizen
der Reihe nach miteinander multipliziert
\begin{equation}
    M = D_0 \cdot P_0 \cdot D_{1} \cdot P_{1} \dots D_{n-1} \cdot P_n \cdot D_n
    \label{eq:Matrixmultiplikation}
\end{equation}
und anschließend genutzt um den Reflexionsgrad R, nach der Definition der
...
später zur Berechnung von Brechungsindizes genutzt wird. Bei diesen optionalen
Parametern handelt es sich um:
\begin{itemize}
    \item einen festen komplexen Brechungsindex \texttt{n},
    \item die Sellmeier-Koeffizient \texttt{B} und\texttt{C} und dem Parameter \texttt{A
              = 0} für leichten Abweichungen,
    \item eine benutzerdefinierte Funktion in Form von string\texttt{formula}, welche
          später mit \texttt{eval(formula)} verwertet wird,
    \item oder eine Messwerttabelle \texttt{table} mit den Spalten \texttt{Wellenlänge} |
          \texttt{Realteil-n} | \texttt{Imaginärteil-n}
...
Die Funktion \begin{center}
    \small \texttt{transfer\_matrix(material\_list, d\_list, wavelength, polarization, theta0)}
\end{center}
...
\begin{equation}
    ...
    \label{eq:MatrixmultiplikationAlg}
\end{equation}
...
\begin{lstlisting}[language=Python, caption={Funktion zur Berechnung des Reflexionsgrades}\label{lst:Reflexionsgrad}]
...
Alternativ kann das \texttt{Start\_GUI.bat} Skript für Windows und das
\texttt{Start\_GUI.sh} Skript für Linux genutzt werden, um das Programm und
seine Abhängigkeiten zu installieren und zu starten. Beide Skripte erstellen
wie oben beschrieben eine virtuelle Umgebung für Python, installieren
automatisch die Abhängigkeiten in \texttt{requirements.txt} und starten danach
das Programm.
...
zusätzlich in Abbildung~\ref{fig:GUI} zusehen.
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.5]{./assets/GUI.pdf}
...
Aufschrift Presets zu sehen. Benutzer können hier eine Auswahl von mehreren
...
Nutzer einen Namen für das Material und einen Berechnungstypen angeben kann.
Die gegebenen Optionen sind:
\begin{itemize}
    \item Berechnung durch die Sellmeier-Gleichung,
    \item Angabe eines festen Brechungsindex,
    \item Nutzen einer benutzerdefinierten Formel und
    \item Interpolation von Daten einer Messwertetabelle.
\end{itemize}
Das Bestätigen der Eingabe erstellt ein neues Materialobjekt und fügt dessen
Daten in das \texttt{Material.json} im Root ein, damit sie zusammen mit den
vorgefertigten Materialien für den Nutzer langzeitig gespeichert sind. In
Abbildung~\ref{fig:GUI-neu} ist das Dialogfenster abgebildet.
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.5]{./assets/GUI-neu.pdf}
    \caption{Dialogfenster zum Hinzufügen neuer Materialien}\label{fig:GUI-neu}
\end{figure}
...
diesen mit \texttt{Zurücksetzen} zu leeren. Eine Übersicht zu den Parametern und
validen Eingaben befindet sich in
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|p{9cm}|}
        ...
        geplottet werden soll. Kann auch angegeben werden in der Form: \texttt{1.Wellenlänge-2.Wellenlänge} \\
        \hline
...
jeweiligen Materialien und ihren individuellen Dicken, welche in
Tabelle~\ref{tab:Angabe1} zu sehen sind und soll im sichtbaren Spektrum
simuliert werden. Hierfür nehmen wir einen Wellenlängenbereich von 400 nm bis
800 nm mit einem senkrechten Einfallswinkel. Anschließend betrachten wir den
Reflexionsgrad in Abhängigkeit des Einfallswinkels.
\begin{table}[h]
    \centering
    ...
    \end{tabular}
\end{table}
Das Ergebnis ist in Abbildung~\ref{fig:Ergebnis1-0} dargestellt und zeigt keine physikalischen
...
bloßes Glas trifft, im Graph verglichen werden, um die Verbesserung der Reflexivität
durch die Antireflexbeschichtung zu bestätigen.

Es wird außerdem, wie in Abbildung~\ref{fig:Ergebnis1-1} zu sehen ist, der
Reflexionsgrad in Abhängigkeit vom Einfallswinkel bestimmt. Es wird bis etwa
40$^\circ$ Reflexivität von nahezu 0 beobachtet, was bestätigt, dass die
Entspiegelung optimal simuliert wird. Ab 60$^\circ$ steigt der Reflexionsgrad
steil an, was bei flachen Winkeln zu erwarten ist und dadurch in der Norm
liegt.

Die zweite zu simulierende Schicht befindet sich in Tabelle~\ref{tab:Angabe2}.
Betrachtet wird hierbei ein Hochreflektor im EUV-Bereich. Es wurde versucht für
dieses Schichtsystem bei extremem Ultraviolettlicht optimale Reflexivität zu
garantieren. Wichtig ist hierbei, dass hier 2 Schichten mit jeweils einem hohen
und einem niedrigen Brechungsindex gepaart sind. Um einen hohen Reflexionsgrad
zu erreichen werden meist bis zu 40 bis 60 dieser Schichtpaare aneinander
gereiht, es wird sich im Anschluss aber nur auf 15 Paare beschränkt, da dies
bereits ein repräsentatives Ergebnis liefert.
\begin{table}[htpb]
    \centering
...
\begin{table}[htbp]
    \centering
    \caption{Schichtdicke für EUV-Schicht}\label{tab:Schicktdicke}
    \begin{tabular}{|l |c| c|}
        \hline
        Material & Dicke$^\circ$ \\
        \hline
...
Die gewählten Schichtdicken sind in Tabelle~\ref{tab:Schicktdicke} zu sehen.
Das Ergebnis in Abbildung~\ref{fig:Ergebnis2-0} ist die gewünschte Funktion für
die EUV-Schicht. Der Hochpunkt bei etwa 13.5 nm lässt vermuten, dass die
Simulation, genau wie im ersten Simulationsergebnis, realitätsgetreu ist. In
Abbildung~\ref{fig:Ergebnis2-1} ist dasselbe Schichtsystem in Abhängigkeit vom
Einfallswinkel in Grad dargestellt, dessen steigender Reflexionsgrad bei
0$^\circ$ und ab 75$^\circ$ ebenfalls eine realistische Simulation vermuten
lässt.

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{./assets/Simulationsergebnis 1.pdf}
    \caption{Darstellung von $R(\lambda)$ für die Antireflexbeschichtung bei senkrechtem Einfall}\label{fig:Ergebnis1-0}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{./assets/1. Simluationsergebnis Winkel.pdf}
    \caption{Darstellung von $R(\varphi)$ für die Antireflexbeschichtung bei einer Wellenlänge von 550 nm}\label{fig:Ergebnis1-1}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{./assets/2. Simulationsergebnis.pdf}
    \caption{Darstellung von $R(\lambda)$ für die EUV-Schicht bei Senkrechtem Einfall}\label{fig:Ergebnis2-0}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{./assets/2. Simulationsergebnis Winkel.pdf}
    \caption{Darstellung von $R(\varphi)$ für die EUV-Schicht bei einer Wellenlänge von 13.5 nm}\label{fig:Ergebnis2-1}
\end{figure}
\clearpage
\printbibliography[]
\end{document}